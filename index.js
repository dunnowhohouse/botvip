...
const dashboard = http.createServer(function (_req, res) {
    ...
    res.write("BangDeveloperBot");
    res.end();
});

dashboard.listen(process.env.port || 0);

logger("Trang web máy chủ đã mở ...", "[ Khởi động ]");
...
function startBot(message) {
    (message) ? logger(message, "[ Khởi động ]") : "";
    ...
    child.on("error", function (error) {
        logger("An error occurred: " + JSON.stringify(error), "[ Khởi động ]");
    });
};
////////////////////////////////////////////////
