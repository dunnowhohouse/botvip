...
module.exports.run = async({api,event,args}) => {
    ...
    const prefix = (threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX;
     if (args.length == 0) return api.sendMessage(`Bạn có thể dùng:\n\n${prefix}${this.config.name} user: nó sẽ lấy ảnh đại diện của chính bạn.\n\n${prefix}${this.config.name} user @tennguoitag: nó sẽ lấy ảnh đại diện người bạn tag.\n\n${prefix}${this.config.name} box: nó sẽ lấy ảnh đại diện box của bạn`, event.threadID, event.messageID);
    if (args[0] == "box") {
           if(args[1]){ let threadInfo = await api.getThreadInfo(args[1]);
           let imgg = threadInfo.imageSrc;
       if(!imgg) api.sendMessage(`Ảnh đại diện của [${threadInfo.threadName}] đây:`,event.threadID,event.messageID);
        else var callback = () => api.sendMessage({body:`Ảnh đại diện [${threadInfo.threadName}] đây:`,attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.png"), event.messageID); 
      return request(encodeURI(`${threadInfo.imageSrc}`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
          ...
          if(!img) api.sendMessage(`Ảnh đại diện của [${threadInfo.threadName}] đây:`,event.threadID,event.messageID)
...
if (args[0] == "user") { 
    ...
    var callback = () => api.sendMessage({body:`Ảnh đại diện của [${name}] đây:`,attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID);   
